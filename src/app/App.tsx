...
import {
  ...
  ChevronDown, ChevronUp, Image, Link2, AlignLeft
} from "lucide-react";
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="absolute inset-0 z-50 flex flex-col" style={{ background: "rgba(15,23,42,0.5)" }}>
      <div className="flex-1" onClick={onClose} />
      <div className="bg-white rounded-t-3xl max-h-[85%] flex flex-col">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-blue-50 flex-shrink-0">
          ...
          <button onClick={onClose} className="p-2 rounded-xl bg-slate-100"><X size={16} color="#64748B" /></button>
        </div>
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-blue-50 flex items-center z-40"
      ...
      {tabs.map(tab => {
        ...
        return (
          <button key={tab.screen} onClick={() => onNav(tab.screen)}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full" style={{ background: BLUE }}>
      <div className="flex flex-col items-center gap-6 px-8">
        ...
        <button onClick={onNext} className="mt-8 w-full py-4 bg-white text-blue-600 rounded-2xl font-bold text-base shadow-lg active:scale-95 transition-transform" style={{ fontFamily: "Outfit, sans-serif" }}>
          Get Started
        </button>
      ...
      <div className="flex-1 px-6 pt-6 overflow-y-auto">
        ...
        {!forgot ? (
          <>
            ...
            <button onClick={() => onLogin(role)} className="w-full py-4 rounded-2xl font-bold text-white mt-6 text-base active:scale-95 transition-transform"
            ...
            <div className="bg-blue-50 rounded-2xl p-4 flex gap-3">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">Use your official institution email ID (@college.edu) to access CampusHUB. Contact your administrator if you face login issues.</p>
            </div>
          </>
        ) : (
          <div>
            ...
            <button className="w-full py-4 rounded-2xl font-bold text-white text-base" style={{ background: BLUE, fontFamily: "Outfit, sans-serif" }}>Send Reset Link</button>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {aiTasksData.map((t, i) => {
              ...
              return (
                <button key={i} onClick={() => setTasksDone(prev => done ? prev.filter(x => x !== i) : [...prev, i])}
                  className="flex items-center gap-3 w-full text-left">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${done ? "border-blue-600 bg-blue-600" : "border-slate-300"}`}>
...
const subjectsData = [
  { name: "Data Structures", code: "CS301", icon: Code, color: "#2563EB", unread: 3, lastMsg: "Assignment 3 deadline extended", time: "2m ago" },
  { name: "DBMS", code: "CS302", icon: Database, color: "#7C3AED", unread: 1, lastMsg: "Lab manual uploaded", time: "15m ago" },
  { name: "Operating Systems", code: "CS303", icon: Terminal, color: "#059669", unread: 0, lastMsg: "Quiz on Friday - Ch 5 & 6", time: "1h ago" },
  { name: "Python Programming", code: "CS201", icon: Code, color: "#F59E0B", unread: 7, lastMsg: "New study material added", time: "2h ago" },
  { name: "Machine Learning", code: "CS401", icon: Brain, color: "#EF4444", unread: 0, lastMsg: "Project guidelines shared", time: "Yesterday" },
];
      ...
      <div className="bg-white border-b border-blue-50 sticky top-0 z-40 px-4 pt-12 pb-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div className="absolute right-0 top-10 bg-white rounded-2xl shadow-xl border border-blue-50 z-50 w-48 overflow-hidden">
                  {["Mark all as read", "Mute notifications", "Archived chats", "Settings"].map(item => (
                    <button key={item} onClick={() => setShowMenu(false)}
                      className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-blue-50 font-medium">{item}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="flex items-center bg-blue-50 rounded-xl px-3 gap-2">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)}
            className="flex-1 py-2.5 text-sm text-slate-700 placeholder-slate-400 outline-none bg-transparent" placeholder="Search subjects, messages..." />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 pb-24 space-y-2">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-16 gap-3">
            ...
            <p className="text-slate-400 text-sm font-medium">No subjects found for "{query}"</p>
          </div>
        ) : (
          filtered.map((sub, i) => (
            <button key={i} onClick={() => onSubject(subjectsData.indexOf(sub))} className="w-full text-left">
              <Card className="flex items-center gap-4 px-4 py-3.5">
                ...
                {sub.unread > 0 && (
                  <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: BLUE }}>
                    <span className="text-[10px] text-white font-bold">{sub.unread}</span>
                  </div>
                )}
              </Card>
            </button>
...
const initMessages: ChatMsg[] = [
  { id: 1, sender: "Dr. Priya Menon", role: "staff", text: "Assignment 3 on Graph Algorithms is due tomorrow at 11:59 PM. Please submit via the portal.", time: "9:15 AM", pinned: true } as any,
  { id: 2, sender: "Rahul Kumar", role: "student", text: "Ma'am, the graph traversal question — should we use DFS or BFS?", time: "10:02 AM" },
...
const discussionMessages: ChatMsg[] = [
  ...
  { id: 2, sender: "Divya S", role: "student", text: "Prim's grows the MST from a vertex, Kruskal's adds edges in sorted order. Both give MST but different approaches.", time: "8:25 AM" },
  { id: 3, sender: "Karthik Raj", role: "student", text: "Oh got it! So Kruskal uses Union-Find right?", time: "8:28 AM" },
  { id: 4, sender: "Arjun Sharma", role: "me", text: "Yes exactly, Union-Find for cycle detection in Kruskal.", time: "8:30 AM" },
];

const materials = [
  { name: "DS_Graph_Algorithms.pdf", size: "2.4 MB", date: "Mar 12", type: "pdf" },
  { name: "Unit3_Trees_Notes.pdf", size: "1.8 MB", date: "Mar 5", type: "pdf" },
  { name: "Practice_Problems_Set4.pdf", size: "0.9 MB", date: "Feb 28", type: "pdf" },
  { name: "DS_Lecture_Slides_Unit4.pptx", size: "5.1 MB", date: "Feb 20", type: "ppt" },
  { name: "Reference_Link_GeeksforGeeks", size: "", date: "Mar 10", type: "link" },
];

function ChatSubject({ subjectIdx, onBack }: { subjectIdx: number; onBack: () => void }) {
  const sub = subjectsData[subjectIdx];
  const [tab, setTab] = useState(0);
  const [msgs, setMsgs] = useState<ChatMsg[]>(initMessages);
  const [discMsgs, setDiscMsgs] = useState<ChatMsg[]>(discussionMessages);
  ...
  const currentMsgs = tab === 0 ? msgs : discMsgs;
  const setCurrentMsgs = tab === 0 ? setMsgs : setDiscMsgs;

  const visibleMsgs = searchQ
    ? currentMsgs.filter(m => m.text.toLowerCase().includes(searchQ.toLowerCase()))
    : currentMsgs;
  ...
  const attachFile = (type: string) => {
    setShowAttach(false);
    const time = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
    const name = type === "pdf" ? "Assignment_3_Arjun.pdf" : type === "image" ? "screenshot.png" : type === "link" ? "https://github.com/arjun/ds-assignment" : "document.docx";
    setCurrentMsgs(prev => [...prev, { id: Date.now(), sender: "Arjun Sharma", role: "me", text: name, time, file: true, fileName: name }]);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F4FF]">
        ...
        {showSearch ? (
          ...
          <div className="flex items-center gap-3 px-4 pt-12 pb-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400">{sub.code} • Dr. Priya Menon • 42 students</p>
            </div>
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div className="absolute right-0 top-10 bg-white rounded-2xl shadow-xl border border-blue-50 z-50 w-44 overflow-hidden">
                  {["View Members", "Pinned Messages", "Search Messages", "Mute Group", "Leave Group"].map(item => (
                    <button key={item} onClick={() => setShowMenu(false)}
                      className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-blue-50 font-medium">{item}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="flex px-4 pb-1 gap-1">
          {["Announcements", "Discussions", "Materials"].map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              className="flex-1 text-xs font-semibold py-2 border-b-2 transition-all"
              style={tab === i ? { borderColor: BLUE, color: BLUE } : { borderColor: "transparent", color: "#94A3B8" }}>
              {t}
            </button>
          ))}
        </div>
      ...
      {tab === 2 ? (
        <div className="flex-1 overflow-y-auto px-4 py-3 pb-24 space-y-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide px-1">Uploaded Materials</p>
          {materials.map((m, i) => (
            <Card key={i} className="flex items-center gap-3 px-4 py-3">
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-3 pb-2 space-y-3">
            {searchQ && (
              <p className="text-xs text-slate-400 font-medium px-1">{visibleMsgs.length} result{visibleMsgs.length !== 1 ? "s" : ""} for "{searchQ}"</p>
            )}
            {visibleMsgs.map(m => (
          ...
          {/* Input */}
          <div className="bg-white border-t border-blue-50 px-3 py-3 mb-16 relative">
            {showAttach && (
              <div className="absolute bottom-16 left-3 right-3 bg-white rounded-2xl shadow-xl border border-blue-100 p-3 grid grid-cols-4 gap-3 z-50">
                {[
                  { icon: FileText, label: "Document", type: "doc", color: "#2563EB" },
                  { icon: Image, label: "Image", type: "image", color: "#059669" },
                  { icon: Link2, label: "Link", type: "link", color: "#7C3AED" },
                  { icon: Globe, label: "PDF", type: "pdf", color: "#EF4444" },
                ].map(a => (
                  <button key={a.type} onClick={() => attachFile(a.type)} className="flex flex-col items-center gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowAttach(!showAttach)} className={`p-2 rounded-xl flex-shrink-0 ${showAttach ? "" : "bg-blue-50"}`} style={showAttach ? { background: BLUE } : {}}>
              ...
              <div className="flex-1 flex items-center bg-blue-50 rounded-xl px-3">
                <input value={msg} onChange={e => setMsg(e.target.value)}
                  ...
                  placeholder={tab === 0 ? "Write announcement..." : "Ask a doubt or reply..."}
                  className="flex-1 py-2.5 text-sm text-slate-700 placeholder-slate-400 outline-none bg-transparent" />
              </div>
              <button onClick={sendMsg} className="p-2.5 rounded-xl flex-shrink-0" style={{ background: msg.trim() ? BLUE : "#E2E8F0" }}>
      ...
      {showMenu && <div className="absolute inset-0 z-40" onClick={() => setShowMenu(false)} />}
      {showAttach && <div className="absolute inset-0 z-40" onClick={() => setShowAttach(false)} />}
    </div>
  );
}
...
const hackathonsData = [
  {
    name: "TechFest 2024 - AI Innovation",
    organizer: "IIT Bombay",
    theme: "Artificial Intelligence",
    regDeadline: "Mar 20, 2024",
    eventDate: "Apr 5-7, 2024",
    prize: "₹5,00,000",
    teamSize: "2-4",
    ...
    aiSummary: "AI-focused hackathon by IIT Bombay. Build innovative ML solutions for real-world problems. Great for CS students with ML background.",
  },
  {
    name: "CampusCode Sprint",
    organizer: "SVCE Internal",
    theme: "Web Development",
    regDeadline: "Mar 16, 2024",
    eventDate: "Mar 22, 2024",
    prize: "₹50,000",
    teamSize: "2-3",
    ...
    url: "",
    aiSummary: "Internal college hackathon open to all years. Perfect for beginners wanting to build their first web project and gain experience.",
  },
  {
    name: "Smart India Hackathon",
    organizer: "Ministry of Education",
    theme: "Government Problem Statements",
    regDeadline: "Apr 1, 2024",
    eventDate: "Apr 20-21, 2024",
    prize: "₹1,00,000",
    teamSize: "6",
    eligibility: "UG Students",
    difficulty: "Intermediate",
    skills: ["IoT", "AI", "Blockchain"],
    type: "External",
    countdown: "18 days left",
    url: "https://www.sih.gov.in",
    aiSummary: "National-level hackathon with government problem statements. High prestige, 6-member teams, diverse tech domains.",
  },
  {
    name: "HackWithIndia 2024",
    organizer: "Devfolio",
    theme: "FinTech & Social Impact",
    regDeadline: "Apr 10, 2024",
    eventDate: "Apr 25-26, 2024",
    prize: "₹2,50,000",
    teamSize: "2-5",
    eligibility: "UG / PG Students",
    difficulty: "Intermediate",
    skills: ["Web3", "React", "Python"],
    type: "External",
    countdown: "27 days left",
    url: "https://devfolio.co",
    aiSummary: "Devfolio-organized event focusing on FinTech and social impact. Great for building portfolio projects with real-world applications.",
  },
];

const difficultyColors: Record<string, string> = { Beginner: "#10B981", Intermediate: "#F59E0B", Advanced: "#EF4444" };

function HackathonScreen({ onNav }: { onNav: (s: Screen) => void }) {
    ...
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      <div className="bg-white border-b border-blue-50 sticky top-0 z-40 px-4 pt-12 pb-3">
        ...
        <div className="flex items-center bg-blue-50 rounded-xl px-3 gap-2 mb-3">
          <Search size={15} color="#94A3B8" />
          <input value={query} onChange={e => setQuery(e.target.value)}
            className="flex-1 py-2.5 text-sm placeholder-slate-400 outline-none bg-transparent text-slate-700" placeholder="Search by name, theme, skills..." />
          {query && <button onClick={() => setQuery("")}><X size={13} color="#94A3B8" /></button>}
          <button onClick={() => setShowFilter(!showFilter)} className={`p-1.5 rounded-lg ${filterDiff.length > 0 ? "" : "bg-white"}`} style={filterDiff.length > 0 ? { background: BLUE } : {}}>
            ...
            <div className="flex gap-2">
              {["Beginner", "Intermediate", "Advanced"].map(d => (
                <button key={d} onClick={() => toggleDiff(d)}
                  ...
                  style={filterDiff.includes(d) ? { background: difficultyColors[d], color: "white" } : { background: "white", color: "#64748B" }}>
                  {d}
                </button>
              ))}
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 pb-24 space-y-3">
        {filtered.length === 0 && (
          <div className="flex flex-col items-center py-16 gap-3">
            ...
            <p className="text-slate-400 text-sm font-medium">No hackathons match your search</p>
            <button onClick={() => { setQuery(""); setFilterDiff([]); }} className="text-xs font-bold" style={{ color: BLUE }}>Clear filters</button>
          </div>
        )}
        {filtered.map((h, i) => {
          const idx = hackathonsData.indexOf(h);
          return (
            <Card key={i} className="overflow-hidden">
              <div className="p-4">
                ...
                <button
                  onClick={() => {
                    if (h.type === "Internal") { setShowRegModal(idx); }
                  ...
                  Register Now {h.type === "External" && <ExternalLink size={13} color="white" />}
                </button>
              </div>
            </Card>
          );
      ...
      {showRegModal !== null && (
        <Modal title="Register for CampusCode Sprint" onClose={() => setShowRegModal(null)}>
          <div className="space-y-3 pt-3">
            <div className="bg-blue-50 rounded-xl p-3">
              <p className="text-xs font-bold text-blue-700 mb-1">Internal Hackathon Registration</p>
              <p className="text-xs text-slate-600 leading-relaxed">Fill in your team details to register for the internal hackathon. The coordinator will confirm your spot via email.</p>
            </div>
            {["Team Name", "Team Leader Name", "Team Members (comma-separated)", "Project Idea (brief)"].map(f => (
              <div key={f}>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1 block">{f}</label>
                {f.includes("brief") ? (
                  <textarea className="w-full bg-blue-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 outline-none resize-none h-16" placeholder={`Enter ${f.toLowerCase()}...`} />
                ) : (
                  <input className="w-full bg-blue-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 outline-none" placeholder={`Enter ${f.toLowerCase()}...`} />
                )}
              </div>
            ))}
            <button className="w-full py-3 rounded-xl font-bold text-white text-sm mt-2" style={{ background: BLUE }}
...
const jobsData = [
  { company: "Google", role: "SWE Intern", type: "Internship", stipend: "₹80,000/month", location: "Bangalore", mode: "Hybrid", skills: ["Python", "Algorithms", "System Design"], deadline: "Mar 25, 2024", eligibility: "3rd/4th Year CSE", match: 92, logo: "G", logoColor: "#4285F4" },
  { company: "Microsoft", role: "Azure Cloud Intern", type: "Internship", stipend: "₹70,000/month", location: "Hyderabad", mode: "Onsite", skills: ["Azure", "Python", "DevOps"], deadline: "Apr 5, 2024", eligibility: "3rd Year CSE/IT", match: 85, logo: "M", logoColor: "#00A4EF" },
  { company: "Flipkart", role: "Data Science Intern", type: "Internship", stipend: "₹50,000/month", location: "Bangalore", mode: "Remote", skills: ["ML", "Python", "SQL"], deadline: "Mar 30, 2024", eligibility: "2nd/3rd Year", match: 78, logo: "F", logoColor: "#2874F0" },
  { company: "Infosys", role: "Full Stack Developer", type: "Full-time", stipend: "₹6.5 LPA", location: "Chennai", mode: "Onsite", skills: ["React", "Node.js", "Java"], deadline: "Apr 15, 2024", eligibility: "2024 Graduates", match: 70, logo: "I", logoColor: "#007CC3" },
];
...
function JobSeekScreen({ onNav }: { onNav: (s: Screen) => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      <div className="bg-white border-b border-blue-50 sticky top-0 z-40 px-4 pt-12 pb-3">
        <div className="flex items-center justify-between mb-3">
          <div><h1 className="text-lg font-bold text-slate-900" style={{ fontFamily: "Outfit, sans-serif" }}>Job Seek</h1>
            <p className="text-xs text-slate-400">AI-powered opportunities</p></div>
          <button onClick={() => onNav("student-notifications")} className="p-2 rounded-xl hover:bg-blue-50 relative">
            <Bell size={20} color="#64748B" />
            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white" />
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24 py-3 space-y-3">
        {filtered.map((job, i) => (
          <Card key={i} className="p-4">
            ...
            <div className="flex gap-2">
              <button className="flex-1 py-2.5 rounded-xl border font-semibold text-xs" style={{ borderColor: BLUE, color: BLUE }}>Save</button>
              <button className="flex-1 py-2.5 rounded-xl font-bold text-white text-xs flex items-center justify-center gap-1.5" style={{ background: BLUE }}>
...
const weekSchedule = [
  { day: "Mon", classes: 4 }, { day: "Tue", classes: 3 }, { day: "Wed", classes: 5 },
  { day: "Thu", classes: 4 }, { day: "Fri", classes: 3 }, { day: "Sat", classes: 2 },
];
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        <div className="px-4 pb-4">
          <div className="flex items-end gap-4 -mt-10 mb-3">
            <div className="w-20 h-20 rounded-3xl border-4 border-white shadow-md flex items-center justify-center font-bold text-2xl text-white" style={{ background: "#7C3AED", fontFamily: "Outfit, sans-serif" }}>AS</div>
            <div className="pb-1">
          ...
          <Card className="p-4 mb-4">
            <h3 className="font-bold text-slate-700 text-sm mb-3" style={{ fontFamily: "Outfit, sans-serif" }}>Weekly Schedule</h3>
            <div className="flex justify-between items-end">
              {weekSchedule.map((d, i) => (
                <div key={d.day} className="flex flex-col items-center gap-1">
                  <span className="text-[10px] font-bold text-slate-500">{d.classes}</span>
                  <div className="w-7 rounded-lg" style={{ height: d.classes * 10 + 10, background: i === 3 ? BLUE : BLUE_MID }} />
          ...
          <Card className="p-4 mb-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-bold text-slate-700 text-sm" style={{ fontFamily: "Outfit, sans-serif" }}>AI Priority To-Do</h3>
            </div>
            <div className="space-y-2">
              {aiTasksData.map((t, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${i === 2 ? "bg-blue-600" : "border-2 border-slate-200"}`}>
                    {i === 2 && <Check size={9} color="white" />}
                  </div>
                  <p className={`text-xs ${i === 2 ? "line-through text-slate-400" : "text-slate-600"}`}>{t.task}</p>
                </div>
              ))}
            </div>
          </Card>
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        <div className="px-4 pb-4">
          <div className="flex items-end gap-4 -mt-10 mb-3">
            <div className="w-20 h-20 rounded-3xl border-4 border-white shadow-md flex items-center justify-center font-bold text-2xl text-white" style={{ background: BLUE, fontFamily: "Outfit, sans-serif" }}>PM</div>
            <div className="pb-1">
              <h2 className="font-bold text-slate-800 text-lg" style={{ fontFamily: "Outfit, sans-serif" }}>Dr. Priya Menon</h2>
              <p className="text-slate-500 text-xs">priya.menon@svce.edu</p>
              <Badge label="Associate Professor" color={BLUE} />
            </div>
          </div>
          ...
          <Card className="p-4 mb-4">
            ...
            <div className="space-y-2.5">
              {[
                ...
                { icon: Mail, label: "Email", value: "priya.menon@svce.edu" },
                { icon: Phone, label: "Phone", value: "+91 98400 12345" },
                { icon: Building2, label: "Department", value: "Computer Science & Engineering" },
                { icon: GraduationCap, label: "Designation", value: "Associate Professor" },
          ...
          <Card className="p-4 mb-4">
            ...
            <div className="space-y-2">
              {[
                { sub: "Data Structures", code: "CS301", sec: "Sec A & B", year: "3rd Year", icon: Code, color: BLUE },
                { sub: "Design & Analysis of Algorithms", code: "CS401", sec: "Sec A", year: "4th Year", icon: Brain, color: "#7C3AED" },
                { sub: "DBMS Lab", code: "CS302L", sec: "Sec B", year: "3rd Year", icon: Database, color: "#059669" },
              ].map((s, i) => (
                <div key={i} className="flex items-center gap-3 bg-blue-50 rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-slate-700 truncate">{s.sub}</p>
                    <p className="text-[10px] text-slate-400">{s.code} • {s.sec} • {s.year}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card className="p-4 mb-4">
            ...
            <div className="space-y-2">
              {[
                "Graph Neural Networks for Social Media Analysis — IEEE 2023",
                "Optimised Cache Algorithms for Edge Computing — Springer 2021",
                "Machine Learning in Healthcare Diagnostics — Elsevier 2019",
              ].map((pub, i) => (
...
const notificationsData = [
  ...
  { icon: Trophy, color: "#7C3AED", title: "Hackathon Registration Closing", body: "TechFest 2024 registration closes in 6 days — don't miss it!", time: "1h ago", type: "academic" },
...
function NotificationsScreen({ onBack }: { onBack: () => void }) {
  ...
  const [activeIntervals, setActiveIntervals] = useState<Record<number, string>>({});

  const tabFilter: Record<NotiTab, (n: typeof notificationsData[0]) => boolean> = {
    All: () => true,
    Urgent: n => n.type === "urgent",
    Academic: n => n.type === "academic",
    Jobs: n => n.type === "job",
  };
      ...
      <div className="bg-white border-b border-blue-50 sticky top-0 z-40 px-4 pt-12 pb-3">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{visible.length} unread</p>
          </div>
        </div>
        <div className="flex gap-2">
          {(["All", "Urgent", "Academic", "Jobs"] as NotiTab[]).map(t => {
            const counts: Record<NotiTab, number> = {
              All: notificationsData.filter(n => !dismissed.includes(notificationsData.indexOf(n))).length,
              Urgent: notificationsData.filter((n, i) => !dismissed.includes(i) && n.type === "urgent").length,
              Academic: notificationsData.filter((n, i) => !dismissed.includes(i) && n.type === "academic").length,
              Jobs: notificationsData.filter((n, i) => !dismissed.includes(i) && n.type === "job").length,
            };
            return (
              <button key={t} onClick={() => setTab(t)}
                ...
                {t}
                {counts[t] > 0 && (
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold ${tab === t ? "bg-white/30 text-white" : "bg-red-100 text-red-500"}`}>
                    {counts[t]}
                  </span>
                )}
              </button>
          ...
          <Card key={n.origIdx} className={`flex items-start gap-3 p-4 ${n.type === "urgent" ? "border-red-100" : ""}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <div className="flex gap-1">
                  {["1h", "1d", "3d", "1w"].map(iv => (
                    <button key={iv} onClick={() => setActiveIntervals(prev => ({ ...prev, [n.origIdx]: iv }))}
                      className="px-1.5 py-0.5 rounded-full text-[10px] font-bold transition-all"
                      style={activeIntervals[n.origIdx] === iv ? { background: BLUE, color: "white" } : { background: "#EFF6FF", color: BLUE }}>
                      {iv}
                    </button>
                  ))}
                </div>
        ...
        <Card className="p-4 mt-2">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: "Hackathon deadlines", enabled: true },
              { label: "Class reminders", enabled: false },
              { label: "Attendance alerts", enabled: true },
            ].map(item => (
              <div key={item.label} className="flex items-center justify-between">
                <span className="text-xs text-slate-600">{item.label}</span>
                <div className={`w-10 h-5 rounded-full transition-all flex items-center ${item.enabled ? "justify-end" : "justify-start"}`}
...
function StaffHome({ onNav }: { onNav: (s: Screen) => void }) {
  return (
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-24 space-y-4">
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { icon: Plus, label: "Post", color: "#2563EB", action: () => onNav("staff-class") },
              { icon: Upload, label: "Upload", color: "#7C3AED", action: () => onNav("staff-class") },
              { icon: Bell, label: "Notify", color: "#F59E0B", action: () => onNav("staff-notifications") },
              { icon: Trophy, label: "Hackathon", color: "#059669", action: () => {} },
            ].map(a => (
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => onNav("staff-class")} className="text-xs font-bold" style={{ color: BLUE }}>View All</button>
          </div>
          <div className="space-y-2">
            {staffClassesData.map((cls, i) => (
              <button key={i} onClick={() => onNav("staff-class")} className="w-full text-left">
                <Card className="flex items-center gap-4 px-4 py-3.5">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {[{ subject: "DS", task: "Assignment 3 submissions", count: 18, icon: FileText }, { subject: "DAA", task: "Project proposals", count: 4, icon: Target }].map((item, i) => (
              <button key={i} onClick={() => onNav("staff-class")} className="flex items-center gap-3 p-2 bg-amber-50 rounded-xl w-full text-left">
...
const initAssignments: Assignment[] = [
  { id: 1, title: "Assignment 3 - Graph Algorithms", desc: "BFS, DFS traversal with complexity analysis", due: "Mar 15", submitted: 18, total: 62, status: "Active" },
  { id: 2, title: "Assignment 2 - Trees & Heaps", desc: "BST insertion, deletion; Heap operations", due: "Mar 8", submitted: 60, total: 62, status: "Closed" },
];
...
const staffMaterials = [
  ...
  { name: "DS_Lecture_Slides_Unit4.pptx", size: "5.1 MB", date: "Feb 20", views: 42, type: "ppt" },
];
...
function StaffClassRoom({ onBack, onNav }: { onBack: () => void; onNav: (s: Screen) => void }) {
  const [tab, setTab] = useState(0);
  ...
  const [materials, setMaterials] = useState(staffMaterials);
  ...
  const submitPost = () => {
    ...
    setAnnouncements(prev => [{
      id: Date.now(), title: postTitle, body: postBody, type: postType,
      date: "Just now", views: 0, submitted: 0, total: 62
    }, ...prev]);
    setPostTitle(""); setPostBody(""); setShowPostModal(false);
  };

  const submitAssignment = () => {
    if (!aTitle.trim()) return;
    setAssignments(prev => [{ id: Date.now(), title: aTitle, desc: aDesc, due: aDue || "TBD", submitted: 0, total: 62, status: "Active" }, ...prev]);
    setATitle(""); setADesc(""); setADue(""); setShowAssignModal(false);
  };

  const uploadMaterial = () => {
    if (!uName.trim()) return;
    setMaterials(prev => [{ name: uName, size: "—", date: "Just now", views: 0, type: "pdf" }, ...prev]);
    setUName(""); setShowUploadModal(false);
  };
    ...
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      <div className="bg-white sticky top-0 z-40">
        <div className="flex items-center gap-3 px-4 pt-12 pb-3 border-b border-blue-50">
          <button onClick={onBack} className="p-2 rounded-xl hover:bg-blue-50"><ArrowLeft size={20} color={BLUE} /></button>
          <div className="flex-1">
            <h2 className="font-bold text-slate-800 text-sm" style={{ fontFamily: "Outfit, sans-serif" }}>Data Structures</h2>
            <p className="text-xs text-slate-400">CS301 • Sec A & B • 62 students</p>
          </div>
                      ...
                      {a.submitted !== undefined && (
                        <>
                          ...
                          <button onClick={() => setTab(3)} className="flex items-center gap-1 px-2 py-1 bg-amber-50 rounded-lg text-xs font-semibold text-amber-600">
                            <Clock size={12} /> {(a.total || 62) - (a.submitted || 0)} pending
                          </button>
                        </>
                      )}
                    </div>
            ...
            {materials.map((f, i) => (
              <Card key={i} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{f.size}{f.size !== "—" ? " • " : ""}{f.date} • {f.views} views</p>
                </div>
                <button className="p-2 bg-blue-50 rounded-xl"><ExternalLink size={14} color={BLUE} /></button>
              </Card>
            ))}
          ...
          <>
            ...
            <div className="flex gap-3 mb-1">
              <Card className="flex-1 p-3 text-center">
                <p className="text-xl font-bold" style={{ color: "#10B981", fontFamily: "Outfit, sans-serif" }}>{allStudents.filter(s => s.status === "Submitted").length}</p>
                <p className="text-xs text-slate-400">Submitted</p>
              </Card>
              <Card className="flex-1 p-3 text-center">
                <p className="text-xl font-bold" style={{ color: "#EF4444", fontFamily: "Outfit, sans-serif" }}>{allStudents.filter(s => s.status === "Pending").length}</p>
                <p className="text-xs text-slate-400">Pending</p>
              </Card>
              <Card className="flex-1 p-3 text-center">
                <p className="text-xl font-bold" style={{ color: BLUE, fontFamily: "Outfit, sans-serif" }}>{allStudents.length}</p>
                ...
                <Card key={i} className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {s.link && (
                        <button className="flex items-center gap-1 text-[10px] font-semibold mt-0.5" style={{ color: BLUE }}>
        ...
        <Modal title="New Post" onClose={() => setShowPostModal(false)}>
          <div className="space-y-3 pt-3">
            ...
            <div>
              ...
              <input value={postTitle} onChange={e => setPostTitle(e.target.value)} placeholder="e.g. Assignment 4 - Sorting Algorithms"
            ...
            <div>
              ...
              <textarea value={postBody} onChange={e => setPostBody(e.target.value)} placeholder="Write your announcement or assignment description..."
      ...
      {showAssignModal && (
        <Modal title="Create Assignment" onClose={() => setShowAssignModal(false)}>
          <div className="space-y-3 pt-3">
            {[
              { label: "Assignment Title", value: aTitle, set: setATitle, ph: "e.g. Assignment 4 - Sorting" },
              { label: "Description", value: aDesc, set: setADesc, ph: "Describe the task and requirements..." },
              { label: "Due Date", value: aDue, set: setADue, ph: "e.g. Apr 5, 2024 11:59 PM" },
            ].map(f => (
              <div key={f.label}>
      ...
      {showUploadModal && (
        <Modal title="Upload Material" onClose={() => setShowUploadModal(false)}>
          <div className="space-y-3 pt-3">
            <div className="border-2 border-dashed border-blue-200 rounded-2xl p-8 flex flex-col items-center gap-3">
            ...
            <div className="flex gap-2">
              {["PDF", "Notes", "Slides", "Lab Manual", "Reference"].map(tag => (
                <span key={tag} className="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-semibold rounded-full">{tag}</span>
              ))}
            </div>
            <div className="flex gap-2 pt-2">
...
function StaffNotifications({ onBack, onNav }: { onBack: () => void; onNav: (s: Screen) => void }) {
  ...
  const [sent, setSent] = useState<{ title: string; body: string; cls: string; time: string }[]>([
    ...
    <div className="flex flex-col h-full bg-[#F0F4FF]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-24 space-y-3">
        <Card className="p-4">
          ...
          <div className="space-y-3">
            <select value={notifClass} onChange={e => setNotifClass(e.target.value)}
              className="w-full bg-blue-50 rounded-xl px-3 py-2.5 text-sm text-slate-700 outline-none border-0">
...
export default function App() {
  ...
  const handleLogin = (r: "student" | "staff") => {
    setRole(r);
    setScreen(r === "student" ? "student-home" : "staff-home");
  };
  ...
  return (
    <div className="w-full h-full flex items-center justify-center" style={{ background: "#1E293B" }}>
      <div className="relative overflow-hidden bg-[#F0F4FF]"
        ...
        {screen === "student-chat" && (
          <ChatList onNav={nav} onSubject={idx => { setSubjectIdx(idx); setScreen("student-chat-subject"); }} />
        )}
        {screen === "student-chat-subject" && <ChatSubject subjectIdx={subjectIdx} onBack={() => setScreen("student-chat")} />}
        {screen === "student-hackathons" && <HackathonScreen onNav={nav} />}
        ...
        {screen === "staff-home" && <StaffHome onNav={nav} />}
        {screen === "staff-class" && <StaffClassRoom onBack={() => setScreen("staff-home")} onNav={nav} />}
